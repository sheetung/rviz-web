...
C++17 直接使用 roscpp / rclcpp，FastAPI 独立管理配置和视频。当前正式入口使用 v2，已补齐显示、控制发布和连接恢复；旧 Python ROS 后端已移除。
...
产物：`backend/build/ros1/rvizweb_native` 或 `backend/build/ros2/rvizweb_native`。
自定义 ROS 消息工作空间可在基础 ROS 环境之后 source。ROS2 自定义类型需要该工作空间提供的 C++ introspection 和序列化类型支持库；ROS1 可直接读取发布者提供的消息定义。

只编译协议测试，不依赖 ROS：

```bash
cmake -S backend -B /tmp/rviz-v2-protocol -DRVIZWEB_WITH_ROS=OFF
...
运行 `./start.sh`（或 `./start.sh dev`）。日常启动只增量构建所选 ROS 的主程序；未修改源码时复用现有产物，ROS 构建环境变化时重新配置，不运行测试。直接运行 `backend/scripts/build.sh` 或 `./start.sh sync` 仍执行完整构建与测试。脚本同时启动 FastAPI 管理服务和前端；退出时统一清理。需要先安装下述构建依赖。原生端口由 `RVIZWEB_NATIVE_PORT` 控制，默认 8082，管理端口默认 8000。


## 开发启动

先按主项目说明准备 `backend/management/.venv` 和前端依赖。三个终端分别运行：

```bash
# 1. 已 source 对应 ROS 环境；ROS 1 需要可访问的 ROS Master
backend/build/ros2/rvizweb_native --host 127.0.0.1 --port 8082

# 2. FastAPI 管理服务；无需 source ROS
backend/scripts/start-management.sh
...
也可使用 `backend/scripts/start-native.sh`，它复用根目录 `.env` 和 ROS 环境选择逻辑；请确保其中 `ROS_WS_URL` 指向所需 ROS 版本且已构建相应产物。
...
复现时在隔离 ROS 环境中，分别运行原生服务、`backend/build/ros2/ros_fixture` 和：

```bash
backend/management/.venv/bin/python backend/tests/smoke.py \
...
C++ 后端位于 `backend/`；ROS1/ROS2 实现与专属解码位于 `src/adapters/`，公共接口位于 `include/rvizweb/`。FastAPI 管理服务放在同一目录下的 `management/`，只处理配置、视频与系统状态，不加载 ROS Python，不提供旧 ROS HTTP/WebSocket 路由。前端只保留原生协议。

从旧目录版本升级后执行 `./start.sh sync`，再执行 `./start.sh`。新的管理虚拟环境位于 `backend/management/.venv`；用户 `.env`、`rvizweb_configs/` 和 `local_data/` 路径不变。构建仍仅选择当前 ROS 环境，ROS1 相关编译和旧版 CTest 兼容修复保留。

原生版本由 `VERSION` 统一生成启动日志和 capabilities 版本；管理服务有独立的 pyproject 版本。`/api/v1/configs` 等管理接口中的 v1 是接口版本，不表示还运行旧 ROS 后端。Docker 继续搁置，旧容器部署文件已移除。
