...
async def run(args):
    ...
    native_path = str(ROOT/'backend/build/ros2/rvizweb_native')
    native = start('native', [native_path, '--port', str(PORT)], env)
    await ready(native)
    fixture = start('fixture', [str(ROOT/'backend/build/ros2/ros_fixture')], env)
    # Existing byte-fidelity, control, unsubscribe and origin checks.
    smoke = start('baseline', [str(ROOT/'backend/management/.venv/bin/python'), 'backend/tests/smoke.py', '--url', URL, '--middleware', 'ros2'], env)
    ...
    await ready(native)
    fixture = start('fixture-filtered', [str(ROOT/'backend/build/ros2/ros_fixture')], env)
    client = await connect()
    ...
    await ready(native)
    simulator = start('simulator', [str(ROOT/'backend/management/.venv/bin/python'), 'scripts/simulate_uav.py', '--no-config'], env)
