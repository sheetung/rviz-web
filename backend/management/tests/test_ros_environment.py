...
class RosEnvironmentTests(unittest.TestCase):
        ...
        for version in ("1", "2"):
            for imports_ok in (True, False):
                with self.subTest(version=version, imports_ok=imports_ok), tempfile.TemporaryDirectory() as directory:
                    root = Path(directory)
                    (root / "backend" / "management").mkdir(parents=True)
                    (root / "frontend").mkdir()
                    result = subprocess.run(
                        ["bash", "-eu", "-c",
                         'source "$1"; MANAGEMENT_DIR="$2/backend/management"; FRONTEND_DIR="$2/frontend"; '
                         'uv() { printf "uv:%s\\n" "$*"; }; '
                         'npm() { echo npm-called; }; '
    ...
    def test_native_build_only_targets_selected_ros_version(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            scripts = Path(directory) / "backend/scripts"
            scripts.mkdir(parents=True)
            shutil.copy2(ROOT / "backend/scripts/build.sh", scripts / "build.sh")
            for version in ("1", "2"):
                (scripts.parent / f"build/ros{version}").mkdir(parents=True)
                result = subprocess.run(
                    ["bash", "-eu", "-c",
                     'cmake() { printf "%s\\n" "$*"; }; '
