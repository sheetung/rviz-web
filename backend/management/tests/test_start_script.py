...
def test_health_checks_bypass_shell_proxy_settings():
    ...
    assert "--connect-timeout 1 --max-time 2 -fsS" in script


def test_all_server_entrypoints_select_backpressured_websockets():
    import ast

    for path in [START_SCRIPT, PROJECT_ROOT / "backend/scripts/start-management.sh"]:
        script = path.read_text(encoding="utf-8")
        assert "--ws websockets" in script
        result = subprocess.run(["bash", "-n", str(path)], capture_output=True)
        assert result.returncode == 0
    tree = ast.parse((PROJECT_ROOT / "backend/management/app/main.py").read_text())
    calls = [
