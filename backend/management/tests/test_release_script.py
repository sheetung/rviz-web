...
def test_component_versions_are_synchronized_with_their_locks():
    for component in ("frontend", "backend", "management"):
        result = subprocess.run(
            [
                "node",
...
def test_components_can_be_versioned_independently(tmp_path):
    ...
    (temporary_root / "backend" / "management").mkdir(parents=True)
    shutil.copy2(
    ...
    for relative_path in (
        ...
        "backend/management/pyproject.toml",
        "backend/management/uv.lock",
    ):
        shutil.copy2(PROJECT_ROOT / relative_path, temporary_root / relative_path)

    original_backend = (temporary_root / "backend/management/pyproject.toml").read_text(
        encoding="utf-8"
    )
    ...
    assert (temporary_root / "backend/management/pyproject.toml").read_text(
        encoding="utf-8"
    ) == original_backend
    ...
    assert (temporary_root / "backend/VERSION").read_text().strip() == "1.3.1"
    assert (temporary_root / "backend/management/pyproject.toml").read_text() == original_backend
