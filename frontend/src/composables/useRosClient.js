...
export function useRosClient() {
  ...
  const subscribe = (topic, messageType, callback, options = {}) => {
    debugLog(`[useRosClient] 订阅请求: ${topic}, 连接状态: ${connectionStore.isConnected}`)
    
    if (!connectionStore.isConnected) {
      console.warn(`[useRosClient] ❌ 未连接到ROS 服务. 状态: ${connectionStore.connectionStatus}`)
      return null
    }
    
    const success = connectionStore.subscribeTopic(topic, messageType, callback, options)
    debugLog(`[useRosClient] connectionStore.subscribeTopic返回:`, success)
    ...
    if (success) {
      ...
      debugLog(`[useRosClient] ✅ 订阅成功: ${topic}`)
      return subscription
    } else {
      console.error(`[useRosClient] ❌ 订阅失败: ${topic}`)
      return null
    }
  }
  ...
  const unsubscribe = (topicOrSubscription) => {
    ...
    if (typeof topicOrSubscription === 'string') {
      ...
      console.error('[useRosClient] ❌ 无效的取消订阅参数:', topicOrSubscription)
      return false
    }
    
    if (subscription) {
      debugLog(`[useRosClient] 取消订阅: ${topic}`)
      const success = connectionStore.unsubscribeTopic(topic, subscription.callback)
      subscribedTopics.value.delete(topic)
      debugLog(`[useRosClient] ✅ 取消订阅成功: ${topic}`)
      return success
    } else {
      console.warn(`[useRosClient] ⚠️ 未找到订阅: ${topic}`)
      return false
    }
  }
