...
状态：待评审方案，尚未开始 v2 编码。已确认同时支持 ROS 1、ROS 2；采用 C++ 接入 ROS，保留 FastAPI 管理配置与视频。用户要求先提出计划方案。
...
这是建议架构，不是现有实现。C++ 负责高频数据接收、转换、编码和推送；FastAPI 保留配置与视频管理。浏览器通过同一个对外入口按路径访问两个服务，高频数据不经过 Python 再转发。
...
v2 路径为建议契约。迁移期保留现有 v1 路由；前端显式选择 v1/v2，不能在故障时自动换服务并重发控制指令。

C++ 以 C++17 为基线，ROS 1 / ROS 2 使用各自适配器与独立构建目录。网络库候选 Boost.Beast 与 Drogon；在第 0 阶段确定依赖与事件循环边界。本机 ROS 2 Humble 可作为首个验证环境，ROS 1 沿用现有部署目标并补齐 C++ 构建和回放验证。
...
迁移后对业务组件使用 `useRosClient` 一类中性名称，但保留 v1 适配器用于验证和回退。协议迁移必须验证 TF 缓存、断线订阅恢复、发布确认以及自定义数值字段。
...
| 4：切换默认 | 配置迁移、启动脚本、容器、发布包、部署说明 | ROS 1、ROS 2 各自全流程验收，明确回退方法 |
...
v1 保留为迁移参照；v2 首个可运行切片通过验证后再逐步切换默认入口。阶段 1 不宣称已经完成全部 v1 功能迁移。
...
本次只核查了现有代码、已有基准说明与参考项目文档。本机发现 ROS 2 Humble 和 rclcpp；尚未验证 ROS 1 C++ 构建环境。本草案没有新增运行服务，也没有执行机器人控制操作。
