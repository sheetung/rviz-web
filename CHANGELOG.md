...
本项目遵循语义化版本控制。组件可独立发布；本次前端、原生后端及管理服务统一为 `2.0.0`，预发布标签为 `v2.0.0-beta1`。

## [2.0.0-beta1] - 2026-09-16

### Added

- C++ 原生 ROS1 / ROS2 数据服务：二进制点云、里程计、TF、激光、路径、Marker、栅格和动态数值字段。
- 目标、初始位姿和速度发布，白名单、消息校验、提交确认和断线订阅恢复；控制指令不重放。
- 同话题 / 类型 / QoS 共享订阅与编码，独立客户端发送队列、运行诊断与可选点云限频、裁剪和体素采样。
- 基于真实点云地图的独立无人机模拟器，支持 `/goal_pose`、持续扫描与恢复巡航。

### Changed

- `backend/` 统一容纳原生服务；ROS 适配器放在 `src/adapters/`，FastAPI 配置、视频与系统状态放在 `management/`。
- 移除旧 Python ROS 后端、前端 v1 通信分支及旧容器部署文件；Docker 继续搁置。
- 安装、启动、测试、源码打包和版本工具适配新目录，仅构建 `.env` 所选 ROS 版本。
- 前端、C++ 后端和管理服务版本统一为 `2.0.0`；C++ 版本由 `backend/VERSION` 统一生成。

### Fixed

- 保留 ROS1 JsonCpp、Boost 执行器和旧版 CTest 兼容修复。
- 修复系统状态读取及断开原生后端后误显示管理服务旧版本的问题。

升级请先停止旧服务，执行 `./start.sh sync`，然后 `./start.sh`。用户 `.env`、工作区配置及 `local_data/` 路径保持不变。

## [1.5.0] - 2026-09-15

### Changed

- 桌面和平板统一为紧凑工作台：标签侧栏、工具栏收纳、柔和选中态和统一尺寸的分隔拉手。
- 横屏支持分隔栏调宽，竖屏支持底部面板调高；适配动态视口、安全区、数字输入和触屏视频控件。
- 保存当前默认工作区配置，补充 C++ ROS 数据服务与 FastAPI 管理服务的后端 v2 计划。

### Fixed

- 目标选择使用 Pointer Events；双指、手势取消、指针捕获丢失及画布外释放不会发布目标。
